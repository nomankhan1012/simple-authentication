...
exports.register = async (req, res, next) => {
  ...
  try {
    ...
    if(userr){
      res.status(StatusCodes.NOT_FOUND).send("already exist")
    }
    ...
    res.status(StatusCodes.OK).json({sucess:"true",user})
...
exports.forgotPassword = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) {
      res.status(StatusCodes.NOT_FOUND).send("No email could not be sent");
    }
  ...
  res.status(StatusCodes.EXPECTATION_FAILED).send("Email could not be sent");
    }
  } catch (err) {
    next(err);
  }
